...
class TCH:
    ...
    def probability_tf(self, X, w):
      ...
      # linear = tf.nn.relu6(tf.matmul(X, w))
      linear = tf.nn.relu6(tf.matmul(X, w))
      # linear[linear < 0] = 0
      # linear[linear > 1] = 1
    ...
    def compute_loss_tf(self, X, y, w):
      """
          Принимает на вход матрицу весов, вектор ответов, вектор весов и параметр L, 
          влияющий на долю класса 0 в отсекающей гиперплоскости.
          Выдаёт на выход значение функции потерь
      ...
      p1 = tf.reshape(self.probability_tf(X, w), [-1])
      # print(f'Proba is {p1}')
      # p1 = tf.clip_by_value(0.1,1,p1)
      loss = tf.reduce_mean((self.L - (self.L + 1) * y) * p1)
    ...
    def compute_train_loss_class_0_tf(self, w):
        """
        ...
        return self.compute_loss_tf(self.X_train, tf.constant(1, dtype=tf.float32) - self.y_train, w)
    ...
    def compute_train_loss_class_1_tf(self, w):
        """
            Function that we want to minimize, the committee member votes for class 1
        ...
        return self.compute_loss_tf(self.X_train, self.y_train, w)
    ...
    def make_hyperplane(self, class_num, X_train, optim_method, c=0.1, cycle_range=100, disp=False, \
        ...
        if optim_method not in ['Nelder-Mead', 'differential_evolution', 'BFGS', 'CG', 'SLSQP', \
                                                    'COBYLA', 'TNC', 'SGD']:
            raise Exception("Unavailable optimization method, only ['Nelder-Mead', 'differential_evolution', 'BFGS', 'CG', 'SLSQP', 'COBYLA', 'TNC', 'SGD] are available")
        ...
        if optim_method == 'Nelder-Mead':
            ...
            for i in range(cycle_range):
                ...
                start_w = tf.Variable((tf.random.normal([X_train.shape[1],1])) * 0.1)

                opt = tf.keras.optimizers.SGD(learning_rate=0.01)

                if class_num == 1:
                        loss = lambda: self.compute_train_loss_class_1_tf(start_w)

                elif class_num == 0:
                        loss = lambda: self.compute_train_loss_class_0_tf(start_w)
                        
                opt.minimize(loss, var_list=[start_w])

                if class_num == 1:
                        fun = self.compute_train_loss_class_1_tf(start_w)
                elif class_num == 0:
                        fun = self.compute_train_loss_class_0_tf(start_w)
                optim_result.append([fun, start_w])
            
            self.optim_result.append(optim_result)
            optim_result.sort(key=lambda x: x[0])
            # optim_result = [x[1] for x in optim_result][0] #int(0.1 * cycle_range)
            hyperplane_coefficients = optim_result[0][1]
            min_loss_func = optim_result[0][0]
    ...
    def fit(self, X, y, optim_method='Nelder-Mead', cycle_range=100, disp=False, adaptive=True, maxiter=10, xatol=0.3, \
        ...
        while self.X_train.shape[0] > 2 * self.N:
            ...
            if optim_method == 'SGD':
                XX = tf.constant(self.X_train)
                yy = tf.constant(self.y_train)
                self.X_train = tf.cast(XX,dtype=tf.float32)
                self.y_train = tf.cast(yy,dtype=tf.float32)
            ...
            while True:
                ...
                if optim_method == 'SGD' and k == 10:
                    self.X_train = self.X_train.numpy()
                    self.y_train = self.y_train.numpy()
                    hp_weights_class_1 = hp_weights_class_1.numpy()
                    hp_weights_class_0 = hp_weights_class_0.numpy()

                cut_1 = self.cutter(self.X_train, hp_weights_class_1).reshape(-1)
                cut_0 = self.cutter(self.X_train, hp_weights_class_0).reshape(-1)
                # print(f'cut_1 = {cut_1}')
    ...
    def predict_proba(self, X):
        ...
        for hp_num in self.weights_hp.keys():
            ...
            cut = self.cutter(test_, weights).reshape(-1)
